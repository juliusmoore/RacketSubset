...
(define (make_arg x state)
  (if (null? x)
      '()
      (if (pair? x)
          (let ([keyval (car x)] [continuation (cdr x)])
            (if (pair? keyval)
                (cons (car keyval) (make_arg continuation state))
                (error "NEEEEEEEE" x " state : " state))
            )
          (error "NOOOOOO" x " state : " state)))
  )

(define (make_param x state)
  (if (null? x)
      '()
      (if (pair? x)
          (let ([keyval (car x)] [continuation (cdr x)])
            (if (pair? keyval)
                (cons (cadr keyval) (make_arg continuation state))
                (error "NEEEEEEEE" x " state : " state))
            )
          (error "NOOOOOO" x " state : " state)))
  )

(define (gen_let_lambda x state)
  (if (and (pair? x) (equal? (length x) 2))
      (let ([arguments (car x)] [body (cadr x)])
        (if (list? arguments)
      (cons (list 'lambda (make_arg arguments state) body) (make_param arguments state))
      (error "You have caused a calamity2: let on " x " state : " state)))
...
(define (eval_let x state)
 ; (let ([attempt (gen_let_lambda x state)])
   ; (if (equal? (length attempt) 1)
   ;     (execute (car attempt) state)
    ;    (if (equal? (length attempt) 2)
    ;        (execute attempt state)
    ;        (error "Unanticipated let to lambda transformation" x " in state : " state)))
   ; )
  (execute (gen_let_lambda x state) state)
  )
